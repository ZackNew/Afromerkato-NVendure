...
export class StockReportService {
  ...
  async getSoldAmountsSince(ctx: RequestContext, day: Date): Promise<any> {
    const mysqlDatetimeFormat = day.toJSON().slice(0, 19).replace("T", " ");
    const soldSince = await this.connection
      .getRepository(ctx, OrderLine)
      .createQueryBuilder("orderLine")
      .innerJoin(
        "orderLine.productVariant",
        "productVariant",
        "productVariant.enabled AND productVariant.deletedAt IS NULL",
      )
    ...
    for (const data of soldSince) {
      if (
        finalValue[data.productVariantId] == undefined ||
        finalValue[data.productVariantId] == null
      ) {
