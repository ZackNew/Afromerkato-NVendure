...
export class StockChartComponent implements OnInit {
  ...
  createGraph() {
    ...
    const handle = this.elementRef.nativeElement.querySelector("#stockChart");
        ...
        options: {
          ...
          onClick: async (event, elements, chart) => {
            if (elements[0]) {
              ...
              const rangeData =
                myData.xAxisDateRanges[myData.xAxisDateRanges.length - 1 - i];
              this.orderPageRedirector.goToProductsPage(
                event,
                elements,
                chart,
