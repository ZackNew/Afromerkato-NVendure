...
export class SalesChartComponent implements OnInit {
        ...
        options: {
          ...
          onClick: async (event, elements, chart) => {
            if (elements[0]) {
              ...
              this.orderPageRedirector.goToOrderPage(
                event,
                elements,
                chart,
                rangeData,
  ...
  async load(graphData: GraphData) {
    this.gqlFormat = this.gqlFormat.replace(
      "%salesState",
      `state: {eq: "${this.salesState}"},`,
    );
  ...
  toggleDropdown(event: any) {
    const div = this.elementRef.nativeElement.querySelector(
      ".btn-group-overflow",
    );
