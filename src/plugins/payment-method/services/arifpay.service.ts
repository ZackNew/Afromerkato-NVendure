import { ConfigService, OrderService, RequestContext, RequestContextService, TransactionalConnection } from "@vendure/core";
...
export class ArifPayServices {
 ...
 async payWithArifPay(ctx:RequestContext,arifPayInput:ArifPayInput):Promise<any>{
     
    const aripayRepo=this.connection.rawConnection.getRepository(Arifpay);
    if(arifPayInput.orderCode){
        const order= await this.orderService.findOneByCode(ctx,arifPayInput.orderCode);           
         if(order){  
           ...
           const orderItems= order.lines.map(orderLine=>{
              return {
                ...
                image:process.env.ECOMMERCE_SERVER_NAME+"/"+"assets"+"/"+orderLine.featuredAsset.preview
          ...
          const response=await axios.post(url,paymentInfo,{headers:{
