...
import { Ctx, Order, OrderService, RequestContext, TransactionalConnection} from '@vendure/core'; 
import { ArifPaySessionStatus, Arifpay } from './entities/arif-pay.entity';

@Controller('payments')
export class ArifPayController {
  constructor(private orderSerice: OrderService,private connection:TransactionalConnection) {}
  ...
  async findAll(@Ctx() ctx: RequestContext, @Body() data: any) {
    ...
    if(data.transaction.transactionStatus=="SUCCESS"){
      const session= await arifpayRepo.findOneBy({sessionId:data.sessionId});
      const order= await this.orderSerice.findOneByCode(ctx,session!.orderCode);
