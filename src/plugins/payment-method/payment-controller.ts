...
export class ProductsController {
  ...
  async findAll(
    ...
    if (ctx.req?.body.decision === "ACCEPT") {
      let order = await this.orderSerice.findOneByCode(
        ctx,
        ctx.req.body.req_reference_number,
      );
      // console.log("the order value is ", order);
      //abyssinia-cybersource-hosted-checkout
      if (order) {
        let updatePaymentMethod = await this.orderSerice.addPaymentToOrder(
          ctx,
          order.id,
          { method: "abyssinia-cybersource-hosted-checkout", metadata: {} },
        );
      }
