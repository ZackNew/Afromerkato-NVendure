import { AdministratorService, EventBus, TransactionalConnection,Transaction, RequestContext, ID, ProcessContext, CustomerEvent, CustomerGroupEvent, CustomerGroupChangeEvent, Administrator } from '@vendure/core';
import {
...
@Injectable()
export class CustomerRelatedActivityLogService  extends ActivityLogService<CustomerEvent|CustomerGroupEvent|CustomerGroupChangeEvent> 
    ...
    @Transaction()
    async registerLog(event: CustomerEvent|CustomerGroupEvent|CustomerGroupChangeEvent){
        ...
        if(event.ctx.apiType === 'admin'){
            ...
            }else if(event instanceof CustomerGroupEvent){
    ...
    onApplicationBootstrap() {
        if(this.processContext.isServer){
            ...
            this.eventBus.ofType(CustomerGroupEvent).subscribe((async (event)=>{
                // console.log('event happened here')
                await this.registerLog(event);
            }));
