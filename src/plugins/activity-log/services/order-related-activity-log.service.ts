...
@Injectable()
export class OrderRelatedActivityLogService
  ...
  implements OnApplicationBootstrap
{
  ...
  async registerLog(
    ...
    if (event.ctx.apiType === "admin") {
      const orderRepo = this.transactionalConnection.getRepository(
        event.ctx,
        Order,
      );
      ...
      if (event instanceof OrderStateTransitionEvent) {
        ...
        if (event.type === "created" || event.type === "deleted") {
          const order = await this.orderService.findOneByOrderLineId(
            event.ctx,
            event.entity.lines[0].id,
          );
          const fulfillmentRepo = getRepository(Fulfillment);
          if (
            !(await fulfillmentRepo.count({
      ...
      if (
        !(
          (event instanceof OrderEvent && event.type === "created") ||
          (event instanceof OrderStateTransitionEvent &&
            event.fromState === "Created" &&
            event.toState === "AddingItems")
        )
      ) {
  ...
  async activityLogs(
    ...
    let allLogsSelectQueryBuilder = this.transactionalConnection
      .getRepository(ctx, OrderRelatedActivityLogEntity)
