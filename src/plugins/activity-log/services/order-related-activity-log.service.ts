import { AdministratorService, EventBus, OrderStateTransitionEvent, TransactionalConnection,Transaction, Ctx, RequestContext, ID, ProductService, Product, UserService, ProcessContext, RefundStateTransitionEvent, FulfillmentStateTransitionEvent, OrderService, FulfillmentEvent, OrderEvent, Fulfillment, Administrator, Order } from '@vendure/core';
import {
        ...
        if(event.ctx.apiType === 'admin'){
            ...
            if(event instanceof OrderStateTransitionEvent){
                ...
                if(event.type === 'created' || event.type === 'deleted'){
                    const order= await this.orderService.findOneByOrderLineId(event.ctx, event.entity.lines[0].id);
                    const fulfillmentRepo= getRepository(Fulfillment);
                    if(! await fulfillmentRepo.count({where:{id: event.entity.id}}) || !order){
                        return;
                    }
            ...
            // const order= await orderRepo.findOne({where:{lines:{id: event.fulfillment.orderItems[0].lineId}, /*select:['id', 'code','lines']*/}});
                const order= await this.orderService.findOneByOrderLineId(event.ctx, event.fulfillment.lines[0].id);    
                if(order){
                    log.type= 'fulfillment_state_transition';
                    log.description= `
                    <a href="./settings/administrators/${currentAdmin!.id}">
