...
export class PaymentMethodActivityLogResolver {
  constructor(
    private paymentMethodActivityLogger: PaymentMethodActivityLogService,
  ) {}

  @Mutation()
  @Transaction()
  async revertPaymentMethodChanges(
  ...
  @Mutation()
  @Transaction()
  async deletePaymentMethodActivityLog(
    @Ctx() ctx: RequestContext,
    @Args("id") id: ID,
  ): Promise<Boolean> {
  ...
  @Query()
  async paymentMethodActivityLogs(
    @Ctx() ctx: RequestContext,
