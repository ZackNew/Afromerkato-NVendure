...
export class CustomerRelatedActivityLogResolver {
  constructor(
    private customerRelatedActivityLogger: CustomerRelatedActivityLogService,
  ) {}

  @Mutation()
  @Transaction()
  async revertCustomerRelatedChanges(
  ...
  @Mutation()
  @Transaction()
  async deleteCustomerRelatedActivityLog(
    @Ctx() ctx: RequestContext,
    @Args("id") id: ID,
  ): Promise<Boolean> {
  ...
  @Query()
  async customerRelatedActivityLogs(
    @Ctx() ctx: RequestContext,
