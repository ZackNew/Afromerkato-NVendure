...
export class ShippingMethodActivityLogResolver {
  constructor(
    private shippingMethodActivityLogger: ShippingMethodActivityLogService,
  ) {}

  @Mutation()
  @Transaction()
  async revertShippingMethodChanges(
  ...
  @Mutation()
  @Transaction()
  async deleteShippingMethodActivityLog(
    @Ctx() ctx: RequestContext,
    @Args("id") id: ID,
  ): Promise<Boolean> {
  ...
  @Query()
  async shippingMethodActivityLogs(
    @Ctx() ctx: RequestContext,
