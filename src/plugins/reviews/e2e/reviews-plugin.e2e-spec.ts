...
import { TEST_SETUP_TIMEOUT_MS, testConfig } from "../e2e/config/test-config";
import { initialData } from "../e2e/config/e2e-initial-data";
import { SubmitProductReviewDocument } from "../e2e/types/generated-shop-types";
import {
  ApproveReviewDocument,
  GetProductReviewDataDocument,
  RejectReviewDocument,
} from "../e2e/types/generated-admin-types";
...
describe("reviews plugin", () => {
  ...
  async function getProductReviewData(id: string) {
    const { product } = await adminClient.query(GetProductReviewDataDocument, {
      id,
    });
